...
/*
    Solution:
        1. get #pos and #neg with one iteration through the input array
            - along the way, put pos nums to pos array
                                 neg      to neg
        2. Start putting pos/neg numbers in turn to the result array
            Who comes first?
                - #pos > #neg: #pos
                - #neg > #pos: #neg 
    Time:
        O(N) when N = input.length
    Space:
        O(N) when N = input.length
*/
const altNumbers = (numArray) => {
  ...
  for (const num of numArray) {
    ...
    else negArr.push(num);
  }

  const result = [];
  let longer = posArr.length >= negArr.length ? posArr : negArr;
  let shorter = posArr.length >= negArr.length ? negArr : posArr;
  let i;
  for (i = 0; i < shorter.length; ++i) {
    result.push(longer[i], shorter[i]);
  }
